...
export default function SettingsPage() {
  ...
  const handleSave = async (endpoint: string, data: any) => {
    try {
      // ✅ milestones ke liye array directly pass karo
      await saveSettings(endpoint, data);
      if (endpoint === "tax") {
        setGstEnabledLocal(!!data?.gst_enabled);
      }
      setToast({ message: "Settings saved!", type: "success" });
      // ✅ Milestones saved ke baad refresh karo taaki IDs sync ho jayein
      if (endpoint === "milestones") {
        const fresh = await import("@/services/settingsService").then(m => m.getAllSettings());
